...
class PurchaseDetailController {
  Future<List<PurchaseLine>> getPurchaseLinesByOrderId(
      int userId, int orderId, String providerName) async {
    final Uri url = Uri.parse(
        '$SERVER_IP/getPurchaseLinesByOrderId/$userId/$orderId/$providerName');
    debugPrint(
        'La URI a la que llamamos es: $SERVER_IP/getPurchaseLinesByOrderId/$userId/$orderId/$providerName');
    ...
    if (res.statusCode == 200) {
      ...
      debugPrint('El resultListJson es: $resultListJson');
      final List<PurchaseLine> resultListPurchase = resultListJson
          .map<PurchaseLine>((json) => PurchaseLine.fromJson(json))
          .toList();
