...
class ManageAddressesState extends State<ManageAddresses> {
  ...
  Future<List<Address>> _getLogisticAdresses() async {
    final Uri url =
        Uri.parse('$SERVER_IP/getLogisticAdresses/${widget.personeId}');
    ...
    if (res.statusCode == 200) {
      ...
      final List<Map<String, dynamic>> resultListJson =
          json.decode(res.body)['data'].cast<Map<String, dynamic>>();
      final List<Address> resultListAddresses = resultListJson
          .map<Address>((json) => Address.fromJson(json))
          .toList();
      Provider.of<DefaultAddressList>(context, listen: false)
          .clearDefaultAddressList();
                ...
                Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) => AddressView(
                            widget.personeId, widget.userId, COME_FROM_DRAWER)
          ...
          if (snapshot.hasData) {
            return ResponsiveWidget(
              smallScreen: _SmallScreenView(
                  snapshot.data!, widget.personeId, widget.userId),
                        ...
                        trailing: PopupMenuButton(
                          ...
                          onSelected: (int result) async {
                            if (result == 1) {
                              ...
                              debugPrint(
                                  'He seleccionado la direccion por defecto');
                              try {
                                _showPleaseWait(true);
                                final Uri url = Uri.parse(
                                    '$SERVER_IP/updateAddress/${addressesList.items[index].addrId}');
                                final http.Response res = await http
                                    .put(url,
                                        ...
                                        body: jsonEncode(<String, String>{
                                          'status_id': 'D',
                                          'addr_id_default': defaultAddressList
                                              .getItem(0)
                                              .addrId
                                              .toString(), //defaultAddressList has always one element (0)
                                ...
                                if (res.statusCode == 200) {
                                  ...
                                  debugPrint(
                                      'Entro en el selected as default address por el 200');
                                  final Address addressListTmp = Address(
                                      addrId:
                                          addressesList.getItem(index).addrId,
                                      streetName: addressesList
                                          .getItem(index)
                                          .streetName,
                                      streetNumber: addressesList
                                          .getItem(index)
                                          .streetNumber,
                                      flatDoor:
                                          addressesList.getItem(index).flatDoor,
                                      postalCode: addressesList
                                          .getItem(index)
                                          .postalCode,
                                      locality:
                                          addressesList.getItem(index).locality,
                                      province:
                                          addressesList.getItem(index).province,
                                      country:
                                          addressesList.getItem(index).country,
                                      state: addressesList.getItem(index).state,
                                      optional:
                                          addressesList.getItem(index).optional,
                                      district:
                                          addressesList.getItem(index).district,
                                      suburb:
                                          addressesList.getItem(index).suburb,
                                      statusId: addressesList
                                          .getItem(index)
                                          .statusId);
                                  Address defaultAddressListTmp = Address(
                                      addrId:
                                          defaultAddressList.getItem(0).addrId,
                                      streetName: defaultAddressList
                                          .getItem(0)
                                          .streetName,
                                      streetNumber: defaultAddressList
                                          .getItem(0)
                                          .streetNumber,
                                      flatDoor: defaultAddressList
                                          .getItem(0)
                                          .flatDoor,
                                      postalCode: defaultAddressList
                                          .getItem(0)
                                          .postalCode,
                                      locality: defaultAddressList
                                          .getItem(0)
                                          .locality,
                                      province: defaultAddressList
                                          .getItem(0)
                                          .province,
                                      country:
                                          defaultAddressList.getItem(0).country,
                                      state:
                                          defaultAddressList.getItem(0).state,
                                      optional: defaultAddressList
                                          .getItem(0)
                                          .optional,
                                      district: defaultAddressList
                                          .getItem(0)
                                          .district,
                                      suburb:
                                          defaultAddressList.getItem(0).suburb,
                                      statusId: defaultAddressList
                                          .getItem(0)
                                          .statusId);
                                  for (var element
                                      in defaultAddressList.items) {
                                  ...
                                  addressesList
                                      .remove(addressesList.getItem(index));
                            ...
                            if (result == 2) {
                              // delete address
                              debugPrint(
                                  'He seleccionado el borrado de la dirección');
                              try {
                                _showPleaseWait(true);
                                final Uri url = Uri.parse(
                                    '$SERVER_IP/deleteAddress/${addressesList.items[index].addrId}');
                                final http.Response res = await http
                                    .delete(url, headers: <String, String>{
                                  'Content-Type':
                                      'application/json; charset=UTF-8',
                                  //'Authorization': jwt
                                }).timeout(TIMEOUT);
