...
class PurchaseDetailView extends StatefulWidget {
  ...
  const PurchaseDetailView(
      this.userId, this.father, this.partnerId, this.userRole,
      {super.key});
...
class PurchaseDetailViewState extends State<PurchaseDetailView> {
  ...
  late List<PurchaseLine>
      itemsPurchase; // (20220517) Angel Ruiz. I need the purchased product to share them
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              onPressed: () async {
                if (itemsPurchase.isNotEmpty) {
                  ...
                  for (var element in itemsPurchase) {
                    textToShare =
                        '$textToShare${element.productId}|${element.newQuantity != -1 ? '${element.newQuantity}(${element.items})' : element.items.toString()}${element.newQuantity != -1 ? (element.newQuantity > 1 ? ' ${element.idUnit}s.' : '${element.idUnit}.') : element.items > 1 ? ' ${element.idUnit}s.' : '${element.idUnit}.'}|${element.productName}\n';
                  }
      ...
      body: FutureBuilder<List<PurchaseLine>>(
          future: _controller.getPurchaseLinesByOrderId(
              widget.userId, widget.father.orderId, widget.father.providerName),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              itemsPurchase = snapshot
                  .data!; // (20220517) Angel Ruiz. I need these data to share them
...
class _SmallScreen extends StatefulWidget {
  ...
  final int partnerId;
  final String userRole;
  const _SmallScreen(this.father, this.itemsPurchase, this.userId,
          ...
          return Card(
            ...
            child: ListTile(
              leading: Container(
                ...
                child: AspectRatio(
                  ...
                  child: CachedNetworkImage(
                    ...
                    imageUrl:
                        '$SERVER_IP$IMAGES_DIRECTORY${widget.itemsPurchase[index].productCode}_0.gif',
                              ...
                              children: <TextSpan>[
                                TextSpan(
                                    text: widget.itemsPurchase[index]
                                        ...
                                        : widget.itemsPurchase[index].items
                                            .toString(),
                                ...
                                TextSpan(
                                    text: widget.itemsPurchase[index]
                                        ...
                                        ? widget.itemsPurchase[index]
                                                    .newQuantity >
                                                1
                                            ? ' ${widget.itemsPurchase[index].idUnit}s.'
                                            : ' ${widget.itemsPurchase[index].idUnit}.'
                                        : widget.itemsPurchase[index].items > 1
                                            ? ' ${widget.itemsPurchase[index].idUnit}s.'
                                            : ' ${widget.itemsPurchase[index].idUnit}.',
                                    style: const TextStyle(
                                        fontWeight: FontWeight.bold))
              ...
              trailing: Column(
                ...
                children: [
                  Expanded(
                    child: (widget.itemsPurchase[index]
                                .possibleStatusToTransitionTo.isNotEmpty &&
                            widget.itemsPurchase[index]
                                    .possibleStatusToTransitionTo[0].priority ==
                                1 &&
                            ...
                            onPressed: () async {
                              try {
                                ...
                                final Uri url = Uri.parse(
                                    '$SERVER_IP/purchaseLineStateTransition/${widget.itemsPurchase[index].orderId}/${widget.itemsPurchase[index].providerName}');
                                ...
                                if (res.statusCode == 200) {
                                  ...
                                  final List<PurchaseStatus>
                                      resultListNextStatus = resultListJson
                                  ...
                                  setState(() {
                                    widget.itemsPurchase[index].allStatus =
                                        widget.itemsPurchase[index]
                                            ...
                                            .elementAt(0)
                                            .statusName;
                                    widget.itemsPurchase[index].statusId =
                                        widget.itemsPurchase[index]
                                            .possibleStatusToTransitionTo
                                            .elementAt(0)
                                            .destinationStateId;
                                    widget.itemsPurchase[index].banPrice =
                                        widget.itemsPurchase[index]
                                            .possibleStatusToTransitionTo
                                            .elementAt(0)
                                            .banPrice;
                                    widget.itemsPurchase[index].banQuantity =
                                        widget.itemsPurchase[index]
                                            .possibleStatusToTransitionTo
                                            .elementAt(0)
                                            .banQuantity;
                                    widget.itemsPurchase[index]
                                            .possibleStatusToTransitionTo =
                                        resultListNextStatus;
                                  });
                                  // process the possibility that the status of the item father could have changed
                                  final List<
                                      Map<String,
                                          dynamic>> resultListJsonFather = json
                                      .decode(res.body)[
                                          'nextStatesToTransitionToItemFather']
                                      .cast<Map<String, dynamic>>();
                                  final List<PurchaseStatus>
                                      resultListNextStatusFather =
                                      resultListJsonFather
                                          .map<PurchaseStatus>((json) =>
                                              PurchaseStatus.fromJson(json))
                                          .toList();
                                  final String statusIdOfTheItemFather = json
                                      .decode(
                                          res.body)['statusIdOfTheItemFather']
                                      .toString(); // status_id of the father item since the father item status could have been changed because have changed the item products of the item father
                                  final String statusNameOfTheItemFather = json
                                      .decode(
                                          res.body)['statusNameOfTheItemFather']
                                      .toString(); // status_name of the father item since the father item status could have been changed because have changed the item products of the item father
                                  final int numStatusOfTheItemFather =
                                      int.parse(json
                                          .decode(res.body)[
                                              'numStatusOfTheItemFather']
                                          .toString()); // num_status of the father item since the father item status could have been changed because have changed the item products of the item father
                                  widget.father.numStatus =
                                      numStatusOfTheItemFather;
                                  widget.father.allStatus =
                                      statusNameOfTheItemFather;
                                  widget.father.statusId =
                                      statusIdOfTheItemFather;
                                  ...
                                  ShowSnackBar.showSnackBar(
                                      context, json.decode(res.body)['message'],
                                      error: true);
                                }
                              } catch (e) {
                                _showPleaseWait(false);
                        ...
                        : Container(
                            padding: EdgeInsets.zero, width: 20, height: 20),
                  ),
                  Expanded(
                    child: (widget.itemsPurchase[index]
                                .possibleStatusToTransitionTo.isNotEmpty &&
                        ...
                        ? PopupMenuButton(
                            ...
                            onSelected: (PurchaseStatus result) async {
                              try {
                                ...
                                debugPrint(
                                    'El valor de result.banPrice es: ${result.banPrice}');
                                debugPrint(
                                    'El valor de result.banQuantity es: ${result.banQuantity}');
                                _showPleaseWait(true);
                                final Uri url = Uri.parse(
                                    '$SERVER_IP/purchaseLineStateTransition/${widget.itemsPurchase[index].orderId}/${widget.itemsPurchase[index].providerName}');
                                final http.Response res = await http
                                ...
                                if (res.statusCode == 200) {
                                  ...
                                  debugPrint(
                                      'El valor de banPrice es: ${widget.itemsPurchase[index].banPrice}');
                                  debugPrint(
                                      'El valor de banQuantity es: ${widget.itemsPurchase[index].banQuantity}');
                                  // process the possibility that the status of the item father could have changed
                                  final List<
                                      Map<String,
                                          dynamic>> resultListJsonFather = json
              ...
              onTap: () async {
                if (widget.itemsPurchase[index].banQuantity == "SI" ||
                    widget.itemsPurchase[index].banPrice == "SI") {
                  final bool purchaseDetailStateChanged = await Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => PurchaseDetailModifyView(
                              widget.userId,
                              widget.father,
                              widget.partnerId,
                              widget.itemsPurchase[index],
                              widget.userRole)));
                  debugPrint(
                      "El valor de purchaseDetailStateChanged es: $purchaseDetailStateChanged");
                  if (purchaseDetailStateChanged) {
                    setState(() {
                      debugPrint(
                          'Estoy dentro de setState. El valor de purchaseDetailStateChanged es: $purchaseDetailStateChanged');
                      debugPrint(
                          'Sigo dentro del setState. El valor de widget.itemsPurchase[index]${widget.itemsPurchase[index].newQuantity}');
                    });
                  }
                }
              },
...
class _MediumScreen extends StatefulWidget {
  ...
  final int partnerId;
  final String userRole;
  const _MediumScreen(this.father, this.itemsPurchase, this.userId,
      this.stateChanged, this.partnerId, this.userRole);
...
class _LargeScreen extends StatefulWidget {
  ...
  final int partnerId;
  final String userRole;
  const _LargeScreen(this.father, this.itemsPurchase, this.userId,
      this.stateChanged, this.partnerId, this.userRole);
