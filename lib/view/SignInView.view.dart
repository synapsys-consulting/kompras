...
class SignInView extends StatelessWidget {
  final String email;
  final int
      reason; //  1 the call comes from the drawer. 2 the call comes from cart.view.dart
...
class _SmallScreenView extends StatefulWidget {
  final String email;
  final int
      fromWhereCalledIs; //  1 the call comes from the drawer. 2 the call comes from cart.view.dart
...
class _SmallScreenViewState extends State<_SmallScreenView> {
  ...
  @override
  Widget build(BuildContext context) {
    final Widget tmpBuilder = Consumer<Cart>(builder: (context, cart, child) {
          ...
          if (_formKey.currentState!.validate()) {
            try {
              ...
              if (res.statusCode == 200) {
                ...
                debugPrint(
                    'El valor de partner_id es: ${payload['partner_id']}');
                ...
                if (payload['partner_id'] != DEFAULT_PARTNER_ID) {
                  ...
                  if (resProducts.statusCode == 200) {
                    ...
                    for (var element in resultListProducts) {
                      ...
                      if (element.productCategoryId !=
                        ...
                        debugPrint(
                            'El product_description retornado desde el API es: ${element.productName}');
                        tmpProductCategoryIdPrevious =
                  ...
                  if (widget.fromWhereCalledIs == COME_FROM_ANOTHER) {
                    ...
                    final Uri urlAddress = Uri.parse(
                        '$SERVER_IP/getDefaultLogisticAddress/${payload['user_id']}');
                    final http.Response resAddress =
                        await http.get(urlAddress, headers: <String, String>{
                      'Content-Type': 'application/json; charset=UTF-8',
                      //'Authorization': jwt
                    });
                    if (resAddress.statusCode == 200) {
                      ...
                      final List<Map<String, dynamic>> resultListJson = json
                          .decode(resAddress.body)['data']
                          .cast<Map<String, dynamic>>();
                      final List<Address> resultListAddress = resultListJson
                    ...
                    Navigator.popUntil(context,
                        ModalRoute.withName('/')); // come from the drawer
                  }
                }
        ...
        child: Container(
          ...
          child: const Center(
            child: Text(
              'Continuar',
              style:
                  TextStyle(fontSize: 24.0, color: tanteLadenBackgroundWhite),
            ),
          ),
        ),
      );
