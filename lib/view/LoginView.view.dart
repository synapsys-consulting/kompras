        ...
        if (_formKey.currentState!.validate()) {
          try {
            ...
            final http.Response res = await http.post(url,
                ...
                body: jsonEncode(<String, String>{'user_name': _email.text}));
            _showPleaseWait(false);
            if (res.statusCode == 200) {
              // Sign in
              debugPrint('El valor de _email.text es: ${_email.text}');
              debugPrint(
                  'El valor de res.body[user_name] es: ${json.decode(res.body)['user_name']}');
              if (!context.mounted) return;
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) =>
                          (SignInView(_email.text, widget.reason))));
      ...
      child: ListView(
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: const Center(
              child: Text('Introduce tu email para continuar con tu pedido.',
                  style: TextStyle(
